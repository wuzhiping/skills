...
| Task | Approach |
|------|----------|
| Read/analyze content | `uv run --project /tdd python -m markitdown presentation.pptx`|
...
```bash
# Text extraction
uv run --project /tdd python -m markitdown presentation.pptx
```
...
Run with: `bun --cwd /tdd $(pwd)/compile.js`
